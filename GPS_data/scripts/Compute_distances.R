...
# ==============================================================================
# --------------- FOR A SAME INDIVIDUAL, DISTANCE THROUGH TIME -----------------
# ==============================================================================


# 1. Define the calculate distance function
calculate_distance <- function(x1, y1, x2, y2) {
  return(sqrt((x2 - x1)^2 + (y2 - y1)^2))
}

# 2. Apply it for each ID and time step
result_per_individual <- df_utm %>%
  group_by(id) %>%
  mutate(
    prev_x = lag(x),
    prev_y = lag(y),
  ...
  dplyr::select(-prev_x, -prev_y)

print(result_per_individual)


# Add seasons
result_per_individual <- result_per_individual %>%
...
# Average distances per season
average_path <- result_per_individual %>% 
...
saveRDS(result_per_individual, "~/GPS_data/Rachel/Df_saves/computed_distance_per_ind.rds")
...
# 4. Save the computations
saveRDS(res, "~/GPS_data/Rachel/Df_saves/computed_distance_interaction_2005_2018.rds")
